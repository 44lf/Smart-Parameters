...
def insert_demo_data(collection, num_samples=50):
    ...
    # 插入数据
    collection.insert([vectors, labels, descs])
    collection.flush()
    collection.load()
    print(f"已插入 {len(vectors)} 条数据（3组相似向量+随机向量）")
    return group0_base_vec
...
def main():
    ...
    for metric in ["L2"]:
        ...
        group0_base_vec = insert_demo_data(coll)

        query_vec = (group0_base_vec + np.random.normal(0, 0.03, dim).astype(np.float32)).tolist()
